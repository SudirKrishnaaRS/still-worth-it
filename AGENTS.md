...
- `src/lib/constants.ts` - every constant and configurable value in the app: localStorage keys, defaults, lookup tables, and any other value a dev might reasonably want to tweak. A new constant is declared and exported here, never inline in the hook/component/util that first needs it - so the whole app's configuration surface is readable from one file instead of scattered across it. Group related constants with a `// --- Section ---` comment, add a doc comment above anything whose purpose isn't obvious from its name alone, and prefer a verbose, self-explanatory constant name (`HOURLY_WAGE_STORAGE_KEY`, not `WAGE_KEY`) over a short one - a dev should understand what a constant is for on first read, without needing to trace where it's used. Constants belonging to one domain (e.g. currency data) may still define their _type_ in that domain's own file (e.g. `CurrencyCode` in `src/lib/currency.ts`) and import it into `constants.ts` - only the values move, not necessarily the types.
